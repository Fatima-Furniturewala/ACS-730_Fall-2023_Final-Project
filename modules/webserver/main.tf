...
data "terraform_remote_state" "networking" { // This is to use Outputs from Remote State
  backend = "s3"
  config = {
    bucket = "group5fatima"            // Bucket from where to GET Terraform State
...
resource "aws_instance" "bastion_vm" {
    ...
    provisioner "file" {
        source      = "Group5Key"
        destination = "/home/ec2-user/Group5Key"
        connection {
            type        = "ssh"
            user        = "ec2-user"
            private_key = file("Group5Key")
            host        = "${self.public_ip}"
        }
    }
    user_data = <<EOF
    #!/bin/bash
    chmod 700 /home/ec2-user/Group5Key
    ls -lart /home/ec2-user/Group5Key > /home/ec2-user/log.txt
    yum -y update
    yum -y install httpd
...
resource "aws_instance" "public_vms_ansible" {
